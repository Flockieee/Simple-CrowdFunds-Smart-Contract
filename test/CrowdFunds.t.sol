...
contract CrowdFundsTest is Test {
    ...
    function setUp() public {
        // Deploy contract
        vm.prank(owner);
        crowdfund = new CrowdFunds(GOAL, owner, DESCRIPTION, DURATION, MIN_FUND);
    ...
    function test_Deployment_Success() public {
        ...
        assertEq(uint256(crowdfund.getStatus()), uint256(CrowdFunds.Status.Active));
    }
    ...
    function test_GetStatus_Ended() public {
        vm.warp(block.timestamp + DURATION + 1);
        assertEq(uint256(crowdfund.getStatus()), uint256(CrowdFunds.Status.Ended));
    }
    ...
    function test_Refund_RevertInsufficientBalance() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(CrowdFunds.INSUFFICIENT_BALANCE.selector, 2 ether, 1 ether));
    ...
    function test_Fallback_Revert() public {
        vm.prank(alice);
        (bool success,) = address(crowdfund).call(abi.encodeWithSignature("nonExistentFunction()"));
